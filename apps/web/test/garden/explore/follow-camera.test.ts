...
describe('followCameraPose', () => {
  it('targets the fox head above the terrain', () => {
    const p = player();
    const { target } = followCameraPose(p, null);
  ...
  it('hangs behind the fox (+z) at yaw 0, looking toward −z', () => {
    const { position, target } = followCameraPose(player(), null);
  ...
  it('keeps the boom length between camera and target', () => {
    const { position, target } = followCameraPose(player({ yaw: 1.1 }), null);
  ...
  it('rotates with yaw', () => {
    ...
    const { position, target } = followCameraPose(player({ yaw: Math.PI / 2 }), null);
  ...
  it('lowers toward fox level when looking up, rises when looking down', () => {
    const up = followCameraPose(player({ pitch: 0.6 }), null);
    const level = followCameraPose(player(), null);
    const down = followCameraPose(player({ pitch: -0.6 }), null);
  ...
  it('clamps boom elevation', () => {
    ...
    const { position, target } = followCameraPose(player({ pitch: -Math.PI / 3 }), null);
    const elev = Math.asin((position.y - target.y) / CAM_BOOM);
    expect(elev).toBeLessThanOrEqual(CAM_ELEV_MAX + 1e-6);
  });

  it('never dips below the water surface / terrain clearance', () => {
    // The pool behind the fox pulls the ground down; force a low boom via pitch up.
    const stream: Stream = {
      level: -0.15,
      points: [
        { x: 4, z: -6, halfWidth: 1.4 },
        { x: 4, z: 2, halfWidth: 5.5 },
        { x: 4, z: 10, halfWidth: 1.4 },
      ],
    };
    const p = player({ x: 4, z: -2, pitch: 0.6 }); // camera lands near the pool edge
    const { position } = followCameraPose(p, stream);
    const floor = surfaceHeightAt(position.x, position.z, stream);
    expect(position.y).toBeGreaterThanOrEqual(floor + CAM_MIN_CLEARANCE - 1e-9);
  });
});
